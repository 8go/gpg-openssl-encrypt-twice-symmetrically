...
# Both "--pinentry-mode loopback" and "--batch" are also required as options, but we use them anyway
PASSPHRASE_FILE_AES_OPTION="--batch --passphrase-file $PASSPHRASE_FILE_AES_FILE"
# If this file exists in local directory use it as source of password for TWOFISH encryption
PASSPHRASE_FILE_TWOFISH_FILE="passphrase-file-twofish" # no spaces please
# Both "--pinentry-mode loopback" and "--batch" are also required as options, but we use them anyway
PASSPHRASE_FILE_TWOFISH_OPTION="--batch --passphrase-file $PASSPHRASE_FILE_TWOFISH_FILE"
...
usage() {
  ...
  echo "${0##*/}: "
  echo "${0##*/}: Script encrypts specified files SYMMETRICALLY (no keys, just two passwords) "
  echo "${0##*/}: appending $FILEEXTENSION to file name."
  echo "${0##*/}: It uses gpg to first encrypt with cipher AES256."
  echo "${0##*/}: Then it uses gpg to encrypt a second time with cipher TwoFish."
  echo "${0##*/}: As a last step, sha512sum is used to compute a 512-bit hash."
  ...
  echo "${0##*/}: command 1: gpg --symmetric --cipher-algo aes256 --digest-algo sha512 --cert-digest-algo sha512 --compress-algo none -z 0 --s2k-mode 3 --s2k-digest-algo sha512 --s2k-count \"$SHA_HASH_ITERATIONS\" --force-mdc --pinentry-mode loopback --no-symkey-cache --output \"plaintext.tmp.enc\" \"plaintext\""
  echo "${0##*/}: command 2: gpg --symmetric --cipher-algo twofish --digest-algo sha512 --cert-digest-algo sha512 --compress-algo none -z 0 --s2k-mode 3 --s2k-digest-algo sha512 --s2k-count \"$SHA_HASH_ITERATIONS\" --force-mdc --pinentry-mode loopback --no-symkey-cache --armor --output \"plaintext.enc\" \"plaintext.tmp.enc\""
  echo "${0##*/}: command 3: sha512sum \"plaintext.enc\" > \"plaintext.sha\""
  echo "${0##*/}: "
...
${0##*/}: Remove old files manually first if you run script twice to encrypt.
${0##*/}: Chosen operation is "encrypt".
${0##*/}: Platform is: LINUX
${0##*/}: No input file provided. Will read plaintext from stdin (keyboard).
${0##*/}: 3 Steps:
${0##*/}:   Step 1: Will ask once for AES256 password.
...
${0##*/}:   Step 3: Will ask once for TwoFish password.
...
${0##*/}: Starting command: gpg \
    ...
    --no-symkey-cache \
    --armor \
    --output "ciphertext.enc" \
    "ciphertext.tmp.enc" # Yes --armor here!
${0##*/}: Success: gpg encrypted file "ciphertext.tmp.enc" successfully.
${0##*/}: Success: shred shredded temporary file "ciphertext.tmp.enc" successfully.
${0##*/}: Starting command: sha512sum "ciphertext.enc" > "ciphertext.sha"
${0##*/}: ciphertext output in file "ciphertext.enc"
${0##*/}: Success: sha512sum hashed file "ciphertext.enc" successfully.
${0##*/}: sha512 hash of ciphertext output in file "ciphertext.sha"
d3d7ff840429d6747c22166b0b4ca0c09fefc7858c15d8b2f7cd646e30e0005695fe02a5032f369f62a9a6eaefbcaf07fb9824a50e40ad6ead25a412f51ee496  ciphertext.enc
${0##*/}: Success: qrencode produced QR code for file "ciphertext.enc" successfully. See "ciphertext.png".
${0##*/}: Success: qrencode produced QR code for file "ciphertext.enc" successfully. See "ciphertext.svg".
...
encryptSymmetricDouble() {
  ...
  if [ "$PASSPHRASE_FILE_AES_OPTION" == "" ]; then
    echo "${0##*/}: First Step : Will ask once for AES256 password."
  fi
  if [ "$#" -eq "0" ]; then # no argument given
    echo "${0##*/}: Next Step  : Will ask for plaintext (text you want to encrypt). There is NO prompt. "
    echo "${0##*/}:              Terminate input with Ctrl-D as first letter in a new line."
  fi
  if [ "$PASSPHRASE_FILE_TWOFISH_OPTION" == "" ]; then
    echo "${0##*/}: Next Step  : Will ask once for TwoFish password."
  fi
  # echo "${0##*/}: Be patient. Password hashing takes more than $HASHING_TIME_IN_SEC seconds each. So wait."

  # Don't do: gpg -e ... "$infile" | gpg -e ... -o "$outfile1"
  command1='gpg \
      --symmetric \
      --cipher-algo aes256 \
      --digest-algo sha512 \
      ...
      --force-mdc \
      --passphrase-repeat 2 \
      '$PASSPHRASE_FILE_AES_OPTION' \
      --pinentry-mode loopback \
      --no-symkey-cache \
      --output "'$outfileT'" \
      "'$infile'" # no --armor'
  echo -e "${0##*/}: Starting command: \n$command1"
  # --passphrase-repeat 2  will be ignored because of  --pinentry-mode loopback
  # but we leave it in there in case of future changes to gpg
  gpg \
    --symmetric \
    --cipher-algo aes256 \
    --digest-algo sha512 \
    ...
    --no-symkey-cache \
    --output "$outfileT" \
    "$infile" # No --armor here!
  ret=$?
  if [ "$ret" -ne "0" ]; then
    echo "${0##*/}: ERROR: gpg returned error \"$ret\". gpg could not encrypt file \"$infile\". Aborting."
    cleanup_exit 10
  else
    echo "${0##*/}: Success: gpg encrypted file \"$infile\" successfully."
  fi
  command2='gpg \
    --symmetric \
    --cipher-algo twofish \
    ...
    --force-mdc \
    --passphrase-repeat 2 \
    '$PASSPHRASE_FILE_TWOFISH_OPTION' \
    --pinentry-mode loopback \
    --no-symkey-cache \
    --armor \
    --output "'$outfile1'" \
    "'$outfileT'" # Yes --armor here!'
  echo -e "${0##*/}: Starting command: \n$command2"
  gpg \
    --symmetric \
    --cipher-algo twofish \
  ...
  ret=$?
  if [ "$ret" -ne "0" ]; then
    echo "${0##*/}: ERROR: gpg returned error \"$ret\". gpg could not encrypt file \"$outfileT\". Aborting."
    cleanup_exit 11
  else
    echo "${0##*/}: Success: gpg encrypted file \"$outfileT\" successfully."
  fi
  chmod 600 "$outfileT"
  ...
  command3='sha512sum "'$outfile1'" > "'$outfile2'"'
  echo "${0##*/}: Starting command: $command3"
  echo "${0##*/}: ciphertext output in file \"$outfile1\""
  sha512sum "$outfile1" > "$outfile2"
  ret=$?
  if [ "$ret" -ne "0" ]; then
    echo "${0##*/}: ERROR: sha512sum returned error \"$ret\". sha512sum could not hash file \"$outfile1\". Is \"sha512sum\" installed?"
  else
    echo "${0##*/}: Success: sha512sum hashed file \"$outfile1\" successfully."
  fi
  ...
  echo -e "Date: $(date)\GPG version: $(gpg --version)\n\nsha512sum version: $(sha512sum --version)\n\nPlaintext filename: $infile\nCiphertext file: $outfile1\n\ncommand 1: $command1\n\ncommand 2: $command2\n\ncommand 3: $command3\n\nFiles:\n$(ls -lG ${outfile}.*)" > "$outfile5"
  echo "${0##*/}: Metadata is in file \"$outfile5\"."
  echo "${0##*/}: SUCCESS! Look at ciphertext output in file \"$outfile1\"."
  echo "${0##*/}: If you want to see how it was encrypted, then run this command, "
  echo "${0##*/}:     but be aware that you will have to provide both passwords: "
  echo "${0##*/}:     gpg --list-packets \"$outfile1\""
} # encryptSymmetricDouble()
...
decryptSymmetricDouble() {
  ...
  *)
  ...
  if [ ! -f "$infile2" ]; then
    ...
    sha512sum "$infile" > "$outfile2"
    ret=$?
    if [ "$ret" -ne "0" ]; then
      echo "${0##*/}: ERROR: sha512sum returned error \"$ret\". sha512sum could not hash file \"$infile\". Is \"sha512sum\" installed?"
    else
      echo "${0##*/}: Success: sha512sum hashed file \"$infile\" successfully."
    fi
  ...
  gpg \
    --decrypt \
    --batch $PASSPHRASE_FILE_TWOFISH_OPTION \
    --pinentry-mode loopback \
    --armor \
    --output "$outfileT" \
    "$infile" # Yes --armor here!
  ret=$?
  if [ "$ret" -ne "0" ]; then
    echo "${0##*/}: ERROR: gpg returned error \"$ret\". gpg could not decrypt file \"$infile\". Aborting."
    cleanup_exit 16
  else
    echo "${0##*/}: Success: gpg decrypted file \"$infile\" successfully."
  fi
  # 2nd decryption
  # outfile already set above
  gpg \
    --decrypt \
    --batch $PASSPHRASE_FILE_AES_OPTION \
    --pinentry-mode loopback \
    --output "$outfile" \
    "$outfileT" # No --armor here!
  ret=$?
  if [ "$ret" -ne "0" ]; then
    echo "${0##*/}: ERROR: gpg returned error \"$ret\". gpg could not decrypt file \"$outfileT\". Aborting."
    cleanup_exit 17
  else
    echo "${0##*/}: Success: gpg decrypted file \"$outfileT\" successfully."
  fi
...
if ! [[ "$(gpg --version)" =~ "gpg (GnuPG) 2" ]]; then
  echo "${0##*/}: ===================================================================="
  echo "${0##*/}: WARNING: Detected that your version of GPG is not v2."
  echo "${0##*/}: WARNING: Detected version: $(gpg --version)"
  echo "${0##*/}: WARNING: Expected something like: gpg (GnuPG) 2.x.y"
  echo "${0##*/}: WARNING: Will abort now to keep you out of trouble."
  echo "${0##*/}: ===================================================================="
  cleanup_exit 18
else
  echo "${0##*/}: Verified that version 2 of GPG is installed. Passed check."
fi
...
echo "${0##*/}: Install latest version of \"gpg\" (version 2), \"sha512sum\", \"shred\" and \"qrencode\"!"
